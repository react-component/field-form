...
export type DeepObject<Store = any, ParentNamePath extends any[] = []> = {
  [FieldKey in keyof Store]:
    | (ParentNamePath['length'] extends 0 ? FieldKey : never)
    | [...ParentNamePath, FieldKey]
    | (Store[FieldKey] extends (number | string)[]
...
export type DeepNamePath<Store = any, ParentNamePath extends any[] = []> = Store extends
  | string
  | number
  | (string | number)[]
  ? Store
  : Store extends Record<string, any>[]
  ? [...ParentNamePath, number] | DeepObject<Store[number], [...ParentNamePath, number]>
  : Store extends Record<string, any>
  ? DeepObject<Store, ParentNamePath>
  : never;
